$.getJSON('data_dartmouth.json', (data) => {
  /** typed.js stuff * */
  const typed = new Typed('#quiz-title', {
  ...
  data.questions.forEach((question) => {
    const current_question = $('<div class="question"></div>');
    ...
    const current_choices = $('<div class="choices"></div>');
    question.choices.forEach((choice) => {
      const current_label = $('<label></label>');
      current_label.append(`<input type="radio" name=${question.name} value=${choice.value}>`);
      const curr_choice = $('<div class="choice"></div>');
      if (choice.img) {
